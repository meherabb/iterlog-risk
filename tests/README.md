...
73 tests across 10 files, all passing. Each file targets one module in `src/bands/` or
...
| File | Checks |
|---|---|
| `test_uniform_band.py` | Theorem 1, Remark 1 — including an exact match to the paper's own worked example (widths 0.034 / 0.118) and a regression test for a real overflow bug found during development |
| `test_lil_lower_bound.py` | Theorem 2 — violation rate climbs monotonically as the band is shrunk below its proven width |
| `test_population_band.py` | Theorem 3, Corollaries 1-3, the DKW embedding — 6 of 8 reference numbers matched exactly against the paper's own tables, plus a direct check that Corollary 3's two-sided certificate is bit-for-bit identical to composing Theorem 3 and Corollary 2 by hand |
